...
def draw_strokes(data, save_root, save_filename, input_img, image_size, init_cursor, infer_lengths, init_width,
    ...
    for round_idx in range(len(infer_lengths)):
        ...
        for round_inner_i in range(round_length):
            ...
            # cv2.imshow('123',1.0-gt_stroke_img_large)
            # cv2.waitKey(0)
            if pen_state == 0:
                canvas += gt_stroke_img_large  # [0.0-BG, 1.0-stroke]
                # cv2.imshow('123',1.0-gt_stroke_img_large)
                # cv2.waitKey(0)
                # cv2.imshow('12', canvas)
                # cv2.waitKey(0)
                gt =np.clip(gt_stroke_img_large, 0.0, 1.0)
                gt = np.round((1.0 - gt) * 255.0).astype(np.uint8)  # [0-stroke, 255-BG]
                g_img = Image.fromarray(gt, 'L')
                g_img.save(f'2/{index}.png', 'PNG')

                ct = np.clip(canvas, 0.0, 1.0)
                ct = np.round((1.0 - ct) * 255.0).astype(np.uint8)  # [0-stroke, 255-BG]
                c_img = Image.fromarray(ct, 'L')
                c_img.save(f'2/{index}canvas.png', 'PNG')
                # cv2.imwrite(f'2/{index}.png', 1.0-gt_stroke_img_large)
                index+=1
            if draw_order:
